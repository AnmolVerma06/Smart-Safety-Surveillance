...
class ThreadClass(QThread):
    ...
    def run(self):
        ...
        while self.ThreadActive:
            ...
            if ret:
                # Model inference
                input_tensor = preprocess(flip_frame).unsqueeze(0)  # Add batch dimension
                with torch.no_grad():
                    output = model(input_tensor)[0]  # Assuming output is a list of detections

                # Post-process output
                bboxes, scores, labels = self.post_process(output)

                # Draw bounding boxes on the frame
                flip_frame = self.draw_bboxes(flip_frame, bboxes, scores, labels)
    ...
    def post_process(self, output):
        # Post-processing steps to extract bounding boxes, scores, and labels
    ...
    def draw_bboxes(self, image, bboxes, scores, labels):
